...
| **钉钉悟空 App** | 仅用于抓取 `DEAP_API_KEYS`，须已在本机登录 |
...
```bash
pnpm install        # 安装依赖
pnpm capture-key    # 抓取 DEAP_API_KEYS 写入 .env（首次 / 密钥过期时，需输一次 sudo）
...
| 变量 | 默认值 | 说明 |
|------|--------|------|
| `DEAP_API_KEYS` | *空（必填）* | 密钥池（逗号分隔多个密钥），直连 deap 用；`pnpm capture-key` 自动追加 |
| `AVAILABLE_MODELS` | `dingtalk-auto,claude-opus-4-8,gpt-4o` | `/v1/models` 展示的候选模型（实际可用性由 deap 运行时验证） |
| `PORT` | `19067` | 监听端口 |
| `API_KEY` | *空* | 代理自身的 API 密钥校验（设值后请求需带 `x-api-key` 头） |

`.env` 示例（`pnpm capture-key` 会自动生成）：

```env
DEAP_API_KEYS=sk-xxxx
KEYS_NAME=my-key
```

### 🔑 如何拿到密钥

本服务的密钥池由 `DEAP_API_KEYS` 环境变量管理，每枚 key 是本机已登录的悟空 daemon 调 deap 时挂在请求头里的临时密钥（约 **29 天**有效）。

**一键抓取：`pnpm capture-key`**——它会起 mitmdump、请你输入一次 sudo 密码开系统代理、触发 daemon 发请求、抓到 key 并**追加**到 `.env` 的 `DEAP_API_KEYS` 池中（git-ignored），最后自动还原系统代理。原理与手动排错见 **[docs/CAPTURE_DEAP_KEY.md](./docs/CAPTURE_DEAP_KEY.md)**。

key 过期（约 29 天）后再跑一次 `pnpm capture-key` 续期即可，旧密钥自动保留在池中。
...
| `/user/balance` | GET | 否 | 密钥池状态查询（`isValid` / `remaining`） |

### 关于模型选择

deap 是**多模型网关**：`dingtalk-auto` 路由到通义千问 `qwen3.7-plus`，而 `claude-opus-4-8` / `gpt-4o` 分别走**真 Claude / 真 GPT**（实测可用）。代理**信任客户端请求里的 `model` 字段**直接透传给 deap；若 deap 返回 `403 model not available`（模型名失效），自动兜底到 `dingtalk-auto` 并短期缓存。`/v1/models` 返回 `AVAILABLE_MODELS` 配置的候选列表供客户端发现。

> ⚠️ 第三方模型（claude/gpt）依赖 deap 的动态渠道池，偶发 `550 No available channel`——代理会自动带退避重试（最多 3 次），通常 1-2 次内恢复。
...
A：说明直连 deap 失败——检查 `DEAP_API_KEYS` 是否有有效密钥、网络是否通。
